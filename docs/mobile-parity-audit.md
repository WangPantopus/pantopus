...
| Notifications (T4.1) | Loading / Loaded / Empty / Error | `notifications` | `notifications` | `GET /api/notifications · /unread-count` · `PATCH /api/notifications/:id/read` · `POST /api/notifications/read-all` | Full parity, including row projection. |
...
| Home dashboard | Loading / Loaded / Error | `homeDashboard` ✚ + `contentDetailShell` | `contentDetailShell` | `GET /api/homes/:id` (with public-profile fallback) | iOS added a feature-level id ✚; both reach `contentDetailShell` via archetype. |
