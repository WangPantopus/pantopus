...
final class CollectiveEventSetupViewModelTests: XCTestCase {
    ...
    func testSaveSucceeds() async {
        ...
        r["/api/scheduling/event-types/et1"] = [
            .status(200, body: #"{"eventType":{"id":"et1","name":"Consult","slug":"consult","assignment_mode":"collective","seat_cap":2,"is_active":true},"assignees":[],"questions":[]}"#),
            .status(200, body: #"{"eventType":{"id":"et1","name":"Consult","slug":"consult","assignment_mode":"collective","seat_cap":2,"is_active":true}}"#)
        ]
        let model = vm(r)
        await model.load()
        model.toggle("u2")
