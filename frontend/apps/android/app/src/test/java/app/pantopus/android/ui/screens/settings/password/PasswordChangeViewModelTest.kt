...
class PasswordChangeViewModelTest {
    ...
    @Test fun loadDiscoversHasPasswordFromAuthMethods() = runTest {
    ...
    @Test fun loadOAuthOnlyDoesNotRequireCurrent() = runTest {
        coEvery { account.authMethods() } returns
    ...
    @Test fun isValidRejectsShortPassword() = runTest {
        coEvery { account.authMethods() } returns
    ...
    @Test fun isValidRejectsMismatchedConfirm() = runTest {
        coEvery { account.authMethods() } returns
            NetworkResult.Success(AuthMethodsResponse(hasPassword = true))
        val vm = viewModel()
        vm.load()
    ...
    @Test fun saveSuccessSetsToastAndShouldDismiss() = runTest {
        coEvery { account.authMethods() } returns
            NetworkResult.Success(AuthMethodsResponse(hasPassword = true))
    ...
    @Test fun save401MarksCurrentPasswordFieldWithError() = runTest {
        coEvery { account.authMethods() } returns
            NetworkResult.Success(AuthMethodsResponse(hasPassword = true))
